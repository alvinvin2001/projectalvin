...
export default class EditBarang extends Component {
  ...
  componentDidMount() {
    ...
    onValue(barangRef, snapshot => {
      ...
      this.setState({
        kode: barangItem.kode,
        nama: barangItem.nama,
        kategori: barangItem.kategori,
  ...
  onSubmit = () => {
    if (
      ...
      const barangReferensi = ref(
        database,
        'Barang/' + this.props.route.params.id,
      );
      const barang = {
      ...
      update(barangReferensi, barang)
        .then(data => {
          Alert.alert('Success', 'Barang Terupdate');
